...
- phone bridge も既定では `127.0.0.1` に bind します。
- スマホはローカルネットワークURLではなく、token付きの外用URLから接続します。
...
認証なしの Codex app-server や bridge を local network や public interface に直接 bind しないでください。
